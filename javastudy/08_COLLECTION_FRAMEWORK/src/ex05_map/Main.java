...
public class Main {
	...
	public static void hashmap1() {
		...
		// HashMap 생성, 자동완성
		Map<String, String> dict = new HashMap<String, String>();  // 뒷 꺽쇠는 생략 가능
		
		//key는 Set로 작동해서 중복도 안되고, 순서도 없다.
		
		//추가1. 새로운 key 사용
		dict.put("apple", "사과"); // key, value
		dict.put("banana", "바나나");
		
		
		//추가2. 기존 key 사용 -> 덮어쓰기(수정)
		dict.put("apple", "아이폰 만드는 회사");
		// 사과에서 아이폰 만드는 회사로 바뀐다.
	...
	public static void hashmap2() {
		...
		// '키'를 이용해서 '값'을 알아내기
		System.out.println("kor 점수는 " + sungjuk.get("kor") + "점");
		System.out.println("eng 점수는 " + sungjuk.get("eng") + "점");
		System.out.println("math 점수는 " + sungjuk.get("math") + "점");
	}
	...
	public static void hashmap3() {
		...
		person.put("age", String.valueOf(age)); //age를 String으로 바꿔야 함.     문자열로 바꾸는 두 가지 방법들.
		person.put("height", height + ""); 		//height를 String으로 바꿔야 함.
		
		// 키를 모두 알아내기
		// 키는 Set 자료구조이다.
		Set<String> keys = person.keySet(); // 키를 Set로 만들어주는 메소드
	...
	public static void treemap1() {
		...
		// 2. 왼쪽은 작은 값, 오른쪽은 큰 값이 저장된다. (부모가 5라면 왼쪽은 5보다 작은 값, 오른쪽은 5보다 큰 값)
		
		// TreeMap
		// 1. key를 기준으로 왼쪽은 작은 값, 오른쪽은 큰 값이 저장된다.
		// 2. 범위 연산(이상, 이하, 초과, 미만)이 필요한 경우 사용한다.
		// 3. 자동으로 정렬되면서 저장된다. ( 검색에 용이)
		// 4. TreeMap 타입으로 선언하면 범위 연산을 쉽게 호출할 수 있다. ( TreeMap implements Map)
		
		TreeMap<Integer, String> people = new TreeMap<Integer, String>();
		people.put(45, "민경태");
		people.put(26, "김두식");
		people.put(55, "최백호");
		people.put(25, "박찬들");  //출력해보면 key 값 순서대로 저장된 것을 알 수 있다.
		
		//확인 -> 자동으로 key의 크기 순으로 저장되어 있다.
