...
/*
	...
	3. URL 구성
	   프로토콜://호스트:포트번호/ContextPath/URLMapping  (다양한 URL Mapping을 등록했다면 그 모든 url mapping은 한 java파일과 연결된다)
	   http://localhost:9090/SERVLET/HelloServlet
	4. Tomcat이 있어야 실행할 수 있다.
*/

// @WebServlet(URL Mapping 값)
@WebServlet("/HelloServlet")  // URL Mapping 값을 여기서 수정 가능
...
public class HelloServlet extends HttpServlet {
	 ...
	 * 3. service() 메소드
	 *    1) 실제 업무 처리를 할 수 있다.
	 *    2) 업무 처리를 위해서 필요한 것 2가지(요청과 응답) <공부해야하는 부분>
	 *       (1) 요청(request)
	 *           ① 클라이언트(client, 사용자)가 서버(server) 측으로 요청한다.
	 *              클라이언트가 서버 측으로 데이터를 보내는 것도 요청이다. (로그인 요청)
	 *           ② HttpServletRequest request 매개변수가 요청을 처리하는 인스턴스이다. (중요)
	 *       (2) 응답(response)
	 *           ① 서버가 클라이언트에게 응답한다.
	 *           ② 서버가 클라이언트에게 데이터를 보내거나 완성된 화면을 보여준다.
	 *           ③ HttpServletResponse response 매개변수가 응답을 처리하는 인스턴스이다. (중요)
	 */
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		System.out.println("service() 메소드 호출");
		
		// 요청 메소드를 확인해서 get 방식이면 doGet() 메소드 호출,
		//                        post 방식이면 doPost() 메소드 호출하기
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// response는 실행화면을 만드는 인스턴스이다. 실행했을 때 웹 브라우저에 이 한줄에 대한 내용을 볼 수 있다.
		// Served at: /SERVLET
	 ...
	 *       (2) $.ajax({
	 *               type: 'post',
	 *               url: 'http://localhost:9090/SERVLET/HelloServlet'
	 *           });
	 *    3) 아무 일도 하지 않는다.
	 *    4) 업무 처리를 위해서 필요한 2개 정보(요청, 응답)를 모두 doGet() 메소드에 전달한다.
	 *       업무 처리는 실제로 doGet() 메소드에서 이뤄진다. 작업의 일원화를 위해 모은 것이다.
	 */
