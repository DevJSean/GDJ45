...
<%
	...
	String ip = request.getHeader("X-Forwarded-For"); // 어딘가를 거쳐서 왔다면 ip가 X-Forwarded-For에 있다.
	if(ip == null) {
		ip = request.getRemoteAddr(); //거쳐서 오지 않았다면 여기에 있음
	}
	...
	/* IPv4 */
	// 127.0.0.1
	// 127_0_0_1 replaceAll("\\.", "_")   // 정규식에서 .는 모든 문자를 의미한다. 자바에서 역슬래시(\)는 두 개를 써야 인식
	
	String filename = ip.replaceAll(":", "_") + "_" + writer + "_" + created + ".txt";
	
	// 저장 경로
	// 서버(tomcat) 내 저장 경로는 우리가 알고 있는 경로와 다르다.
