...
<script>
	$(document).ready(function(){
		...
		$('#filename').change(function(){
			...
			// substring(beginIndex); // 인덱스 정보부터 끝까지 자르기
			// filename.lastIndexOf('.') + 1 // 인덱스 정보
			var extension = filename.substring(filename.lastIndexOf('.') + 1).toLowerCase();
			var accepts = ['jpg', 'jpeg', 'png', 'gif', 'tif', 'tiff'];
			if($.inArray(extension, accepts) == -1) { // extension이 배열 accepts 내부에 없으면 -1 반환
				alert('확장자가 jpg, jpeg, png, gif, tif, tiff인 파일만 첨부할 수 있습니다.');
				$(this).val(''); //첨부된 파일명을 없애면 첨부가 없어진다.
				return;
			}
