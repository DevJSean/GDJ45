...
public class EmpDAO {
	...
	// singleton
	private static EmpDAO empDAO = new EmpDAO(); // 내가 하나 만들었다.
	private EmpDAO() { }  // 나만 만들 수 있다.
	...
	public int insertEmp(Emp emp) {
		...
		try {
			...
			close(con, ps, null); // rs는 사용하지 않아서 null
		}
		return res;
	}
	...
	public List<Emp> selectEmpList() {
		...
		try {
			...
			while(rs.next()) { // 한 줄씩 확인해서 있으면
				// empno, name, dept, hired 칼럼 값 가져와서 넣기
				// Emp emp = new Emp(rs.getLong("EMPNO"), rs.getString("NAME"), rs.getString("DEPT"), rs.getDate("HIRED"));
				Emp emp = new Emp();
	...
	public Emp selectEmpByEmpNo(Long empNo) {
		Emp emp = null;
		try {
			con = dataSource.getConnection(); //커넥션 대여 (DBCP 내용)
			sql = "SELECT EMPNO, NAME, DEPT, HIRED FROM EMP WHERE EMPNO = ?";
			ps = con.prepareStatement(sql);
			ps.setLong(1, empNo);
			rs = ps.executeQuery();
			
			// 검색된 결과가 있거나, 검색된 결과가 없는 경우
			// while문을 사용할 필요가 없다.
			if(rs.next()) {
				emp = new Emp();
				emp.setEmpNo(rs.getLong("EMPNO"));
