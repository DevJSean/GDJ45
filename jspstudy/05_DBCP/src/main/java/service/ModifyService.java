...
public class ModifyService implements EmpService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		// DB에 변경이 있는 경우 redirect 해야 함.
		
		// ActionForward 반환은 어떻게 할 것인가?
		// null 값을 반환한다.
		// 성공/실패 모두 이미 이동했기 때문에 ActionForward를 통한 이동은 불가능하다.
		try {
			PrintWriter out = response.getWriter();
			if(res > 0) {
				out.println("<script>");
				out.println("alert('사원 정보가 수정되었습니다.')");
				//out.println("location.href='/DBCP/list.do'"); // 목록 보기 또는 상세 보기
				out.println("location.href='/DBCP/detail.do?empNo=" + empNo +"'"); // 사원 상세 보기
				out.println("</script>");
				out.flush();
				out.close();
			} else {
		...
		return null;  // 성공/실패 모두 이미 이동했기 때문에 ActionForward를 통한 이동은 불가능하다.
	}
