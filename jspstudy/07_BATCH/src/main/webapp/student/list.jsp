...
<script>
	$(document).ready(function(){
		// this = event 타겟 = <input type="button" value="수정" class="btn_update"> (클릭한 대상)
		// 클릭한 대상의 이전형제 <input type="hidden" name="stuNo" value="${student.stuNo}">
		
		// 상세 버튼의 이전 형제로 학번을 저장해 두고 사용한다.
		$('.btn_detail').click(function(){
			location.href='/BATCH/detail.do?stuNo=' + $(this).prev().val();
			                                          // 클릭한 대상의 이전형제의 value 속성
		});
				...
				<c:if test="${totalCount ne 0}">
					<c:forEach items="${list}" var="student">
						<tr>
							...
							<td>${student.grade}</td>
							<td>
								<!-- 상세 버튼의 이전 형제에 학번을 저장해 두고 사용 -->
								<input type="hidden" name="stuNo" value="${student.stuNo}"> <!-- 반복문이기 때문에 속성에 id를 넣으면 안된다 -->
