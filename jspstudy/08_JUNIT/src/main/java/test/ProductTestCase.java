...
class ProductTestCase {
	...
	void 제품등록테스트() {
		...
		// assertEquals(예상값, 실제 발생한 값, [실패시메시지])
		// expected : 예상값
		// actual   : 실제 발생한 값
		// 예상값과 실제 발생한 값이 같으면 녹색 불, 다르면 빨간 불이 나온다.
		assertEquals(1, res, "제품 등록 실패");
	}
	
	//@Test   //JUnit 단위 테스트를 할 때 실행되는 메소드라는 뜻
	...
	void 제품조회테스트() {
		...
		//assertNotNull(product); // 전달된 product이 null이 아니면 성공 null이면 실패
	...
	void 제품삭제테스트() {
		...
		int res = ProductDAO.getInstance().deleteProduct(26L);
		assertEquals(1, res);
