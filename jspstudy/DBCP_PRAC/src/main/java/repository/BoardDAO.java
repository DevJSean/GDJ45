...
public class BoardDAO {
	...
	public List<Board> selectBoardList(){
		List<Board> list = new ArrayList<Board>(); // null이 넘어올 확률은 없다(내용이 없는 경우는 가능)
		try {
			con = dataSource.getConnection();
			sql = "SELECT * FROM BOARD ORDER BY NO DESC"; // * 쓰면 안된다.
	...
	// 3. 전체게시글 개수 가져오기
	// 리스트의 사이즈로 개수를 구하면 안된다. 페이징 때문에 항상 고정인 경우 때문
	// 무조건 DB를 갔다가 와야 한다.
	public Long selectBoardCount() {
		...
		try {
			...
			if(rs.next()) {
				count = rs.getLong("개수"); // 칼럼의 이름이 개수이다. count = rs.getLong(1)
			} 
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
