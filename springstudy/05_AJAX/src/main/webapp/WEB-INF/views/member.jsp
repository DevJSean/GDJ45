...
<script>
	...
	$(document).ready(function(){
		...
		$('#btn3').on('click', fnAjax3)                   // 위와 같은 뜻임
	...
	function fnAjax1(){
		...
		$.ajax({
			...
			url : '${contextPath}/member/detail1',                    // 매핑 URL(매핑), MemberController에서 매핑 찾기
			...
			dataType : 'text',       // 응답 데이터 타입이 text이다.
	...
	function fnAjax4(){
		...
		$.ajax({
			...
			// JSON 데이터를 주소창에 붙여서 보내지 못한다.(get 방식이 아니라는 뜻)
			// JSON 데이터를 본문에 포함시켜서 보내는 post 방식으로 보내야 한다.
			type: 'post',
			
			// JSON 데이터를 만들어서 보낼 때는
			// JSON 데이터를 문자열 형식으로 만들어서 보낸다.
			data: JSON.stringify({
				'id': $('#id').val(),
				'pw': $('#pw').val()
			}),
			...
			// JSON 데이터를 만들어서 보낼 때는 
			// 보내는 데이터의 타입을 작성해 준다.
			// contentType 이라는 속성으로 작업한다.
			// 요청 데이터 타입을 확인하는 건 자바 측이기 때문에
			// 자바가 사용하는 JSON의 타입인 'application/json'이라고 작성해 줘야 한다.
