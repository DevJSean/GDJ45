...
public class BoardRepository {
	...
	public BoardRepository() {
		...
		try {
			...
			dataSource = (DataSource)context.lookup("java:comp/env/jdbc/oracle"); // java:comp/env/ : 정해져 있는 문구(tomcat 쓸 때 넣는 말)
			
		} catch (NamingException e) {  // context.xml에 등록한 Resource name과 context.lookup에 적은 이름이 다를 때, 혹은 context.xml을 다른 곳에 두었을 때 
	...
	// DB와 직결되는 부분으로 select, insert, update, delete 를 활용하는 것이 좋다.
