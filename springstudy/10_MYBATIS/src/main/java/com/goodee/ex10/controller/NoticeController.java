...
public class NoticeController {
	...
	public String save(HttpServletRequest request, RedirectAttributes redirectAttributes) {    // request를 사용할 거라면 service도 request를 사용해야 한다.

		//request.getRemoteAddr(); // 작성자 ip 알아내기
		
		// 성공/실패 메시지 처리가 없는 경우
		// return "redirect:/notice/list"; // redirect는 매핑으로 이동한다. 목록 보기로 넘어가는 매핑 값 
		...
		// 이용해서 값을 전달할 수 있다.
		redirectAttributes.addFlashAttribute("kind", "insert"); // 작업 종류
		redirectAttributes.addFlashAttribute("res", noticeService.save(request)); // 등록 및 성공/실패 유무 반환
		return "redirect:/notice/afterDML"; // redirect는 매핑으로 이동한다.
	}                     // ↓
	                      // ↓
	...
	public String detail(HttpServletRequest request, Model model) {
		model.addAttribute("notice", noticeService.findNoticeByNo(request)); // 이 때는 조회수 늘리고
		return "notice/detail"; // notice 폴더 아래 detail.jsp로 forward 이동
	}
	
	@GetMapping("/notice/changePage")
	public String changePage(HttpServletRequest request, Model model) {
		model.addAttribute("notice", noticeService.findNoticeByNo(request)); // 이 때는 조회수 늘리면 안 됨
		return "notice/change"; // notice 폴더 아래 change.jsp로 forward 이동.
	}
	...
	public String change(HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("res", noticeService.change(request)); // 수정 및 성공/실패 유무 반환
   ...
   public String transaction() {
	   noticeService.transaction(); // 테스트 용 서비스 호출
