...
import com.goodee.ex14.util.NaverPageUtils;
...
public class GalleryServiceImpl implements GalleryService {
	...
	public void findGalleries(HttpServletRequest request, Model model) {
		...
		// PageEntity 계산
		NaverPageUtils naverPageUtils = new NaverPageUtils();
		naverPageUtils.setPageEntity(totalRecord, page);
		
		// beginRecord + recordPerPage => Map
		Map<String, Object> map = new HashMap<>();
		map.put("beginRecord", naverPageUtils.getBeginRecord() - 1);
		map.put("recordPerPage", naverPageUtils.getRecordPerPage());
		
		// beginRecord부터 recordPerPage 만큼 목록
		List<GalleryDTO> galleries = galleryMapper.selectGalleryList(map);
		
		// list.jsp로 보낼 데이터
		model.addAttribute("totalRecord", totalRecord);
		model.addAttribute("galleries", galleries);
		model.addAttribute("beginNo", totalRecord - (page - 1) * naverPageUtils.getRecordPerPage());
		model.addAttribute("paging", naverPageUtils.getPaging(request.getContextPath() + "/gallery/list"));
	...
	public void findGalleryByNo(HttpServletRequest request, Model model) {
		...
		HttpSession session = request.getSession();
		if(referer.endsWith("list") && session.getAttribute("updateHit") == null) {  // 목록 보기(/gallery/list)에서 제목을 클릭했고, session에 updateHit 속성이 없다면
	...
	public ResponseEntity<byte[]> display(HttpServletRequest request) {
		...
		Long fileAttachNo = Long.parseLong(request.getParameter("fileAttachNo"));
		Optional<String> opt = Optional.ofNullable(request.getParameter("type"));
		String type = opt.orElse("image");
		...
		try {
			...
			entity = new ResponseEntity<>(FileCopyUtils.copyToByteArray(file), HttpStatus.OK);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void saveGallery(MultipartHttpServletRequest multipartRequest, HttpServletResponse response) {
		...
		// FILE_ATTACH 테이블에 INSERT할 galleryNo 정보는 insertGallery()를 수행할 때 gallery에 저장된다.
		...
		for (MultipartFile multipartFile : files) {
			...
			try {
				...
				if(multipartFile != null && multipartFile.isEmpty() == false) {  // 첨부가 있다.(둘 다 필요함)
					...
					// 첨부파일 확인(제거)
					// *** String contentType이 항상 null을 반환하고 있음 ***
		...
		if(attaches != null && attaches.isEmpty() == false) {
			...
			for(FileAttachDTO attach : attaches) {
				...
				try {
					...
					// 썸네일 이미지 삭제 **
					File thumbnail = new File(path, "s_" + saved);
					if(thumbnail.exists()) {
						thumbnail.delete();
					}
		...
		for (MultipartFile multipartFile : files) {
			...
			try {
				...
				if(multipartFile != null && multipartFile.isEmpty() == false) {  // 첨부가 있다.(둘 다 필요함)
					...
					// FileAttach INSERT 수행(첨부 추가의 개념)
					fileAttachResult += galleryMapper.insertFileAttach(fileAttach);
	...
	public void removeFileAttach(HttpServletRequest request, HttpServletResponse response) {
		...
		String path = fileAttach.getPath();
		String saved = fileAttach.getSaved();
		...
		int res = galleryMapper.deleteFileAttach(fileAttachNo);
		...
		try {
			...
			out.println("<script>");
			if(res == 1) {
				out.println("alert('첨부파일이 삭제되었습니다.')");
				out.println("location.href='" + request.getContextPath() + "/gallery/detail?galleryNo=" + galleryNo + "'");
			} else {
				out.println("alert('첨부파일이 삭제되지 않았습니다.')");
				out.println("history.back()");
			}
			out.println("</script>");
			out.close();
