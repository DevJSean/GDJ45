...
public class SecurityUtils {
	...
	public static String authCode(int length) {
		...
		for(int i = 0; i < length; i++) {
			if(Math.random() < 0.5) {
				sb.append( (char)((int)(Math.random() * 10) + 48) ); // 48 ~ 57  : 문자 '0' ~ '9'
				//sb.append( (char)((int)(Math.random() * 10) + '0') ); // 48 ~ 57  : 문자 '0' ~ '9'
			} else {
				sb.append( (char)((int)(Math.random() * 26) + 65) ); // 65 ~ 90 : 문자 'A' ~ 'Z'
				//sb.append( (char)((int)(Math.random() * 26) + 'A') ); // 65 ~ 90 : 문자 'A' ~ 'Z'
			}
		}
		return sb.toString();
	}
	...
	public static String sha256(String password) {
		...
		StringBuilder sb = new StringBuilder();
		for(byte b : bytes) {
			sb.append(String.format("%02X", b)); // 바이트를 문자열로 바꾸기, %X : 대문자로 표시한 16진수, 02 : 2자리로 나타냄
		}
		return sb.toString();
	}
