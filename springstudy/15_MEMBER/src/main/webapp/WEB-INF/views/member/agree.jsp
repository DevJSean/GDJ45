...
<script>
	$(function(){
		...
		$('#checkAll').on('click', function(){
			...
			// 전체 선택이 checked   -> 개별 선택도 checked 상태이고 
			// 전체 선택이 unchecked -> 개별 선택도 unchecked 상태이다.
			// 체크 여부 확인 방법 2가지
			// 1) 확인 attr('checked')   수정 attr('checked', 'checked')
			// 2) 확인 prop('checked')   수정 prop('checked', true)
	
			// 체크박스 체크 상태 변경
			$('.checkOne').prop('checked', $('#checkAll').prop('checked'));
			
			// 배경 이미지(체크 이미지) 변경
			if($('#checkAll').is(':checked')){ // 전체선택이 체크되어 있으면
...
<body>
	...
	<form id="f" action="${contextPath}/member/signInPage">
		...
		<!-- 
		name이 같은 데이터를 보낼 때
		request.getParameter.values("agreements")
		배열로 받아야 한다.
		String[] params = {"location", "promotion"} -->
