...
<script>
	...
	//      이 경우에는 Promise 처리를 해야 한다. ajax 처리의 순서를 지정할 수 있게 된다.
	...
	function fnEmailAuth() {
		fnEmailCheck().then( // then은 resolve() 담당
			function() {
				$.ajax({
					...
					data: 'email=' + $('#email').val(),
					dataType: 'json',
					success: function(obj){   // obj에는 발송한 인증코드(authCode)가 저장되어 있다.
	...
	function fnIdCheck(){
		$('#id').on('keyup', function(){
			...
			if(regId.test($('#id').val())==false){
				$('#idMsg').text('아이디는 소문자 1~32자 사이 입니다.').addClass('dont').removeClass('ok');
				idPass = false;
				return;
			}
			// 아이디 중복 체크
			$.ajax({
				...
				dataType: 'json',
				success: function(obj){
					if(obj.res == null) {  // 중복되는 ID가 DB에 존재하지 않음 == 써도 되는 ID임.
						$('#idMsg').text('멋진 아이디네요!').addClass('ok').removeClass('dont');
						idPass = true;
					} else {  // 중복되는 ID가 DB에 존재함 == 쓰면 안 되는 ID임.
