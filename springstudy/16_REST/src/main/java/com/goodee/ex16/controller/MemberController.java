...
public class MemberController {
	...
	public Map<String, Object> addMember(@RequestBody MemberDTO member, HttpServletResponse response) { 
		return memberService.addMember(member, response); // MemberDTO: json 보내기, HttpServletResponse : 에러 상황의 status, message 만들기
	}
	
	@ResponseBody                // {경로에 포함된 페이지 변수} 
	@GetMapping(value="/members/page/{page}", produces="application/json")
	public Map<String, Object> getMembers(@PathVariable(value="page", required=false) Optional<String> opt){ // @PathVariable (defaultValue가 없어서 Optional로 받는다)
