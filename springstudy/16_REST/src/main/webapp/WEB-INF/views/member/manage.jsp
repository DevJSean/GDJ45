...
<script>
	...
	function fnRemove() {
		$('#btnRemove').on('click', function(){
			...
			for(let i = 0; i < $('.checkOne').length; i++){
				// $('.checkOne')[i].value    // JavaScript 방법
				// $($('.checkOne')[i]).val() // jQuery 방법
			...
			if(deleteCount == checkedCount) {
				alert(deleteCount + '명의 회원이 삭제되었습니다.');
			}
			fnList();
		})
	}
	...
	// 6. 회원조회
	function fnDetail(){
		$(document).on('click', '.btnDetail', function(){ // 동적 요소 클릭 이벤트
			$.ajax({
	...
	function fnCheckOne(){
		$(document).on('click', '.checkOne', function(){ // 동적 요소 선택
			let checkCount = 0;
			for(let i = 0; i < $('.checkOne').length; i++){
	...
	function fnInit(){
		...
		$('#name').val('');
		$(':radio[name="gender"][value="NONE"]').prop('checked', true);
		$('#address').val('');
	}
...
<body>
	...
	<div>
		<input type="hidden" name="memberNo" id="memberNo"><br>
